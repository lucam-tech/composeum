...
class CompositePreviewRegistryTest {
    ...
    fun `merges entries from multiple registries`() {
        val a = object : PreviewRegistry { override val entries = listOf(entry("a")) }
        val b = object : PreviewRegistry { override val entries = listOf(entry("b")) }
    ...
    fun `deduplicates by key - first occurrence wins`() {
        val first = object : PreviewRegistry { override val entries = listOf(entry("dup", name = "First")) }
        val second = object : PreviewRegistry { override val entries = listOf(entry("dup", name = "Second")) }
    ...
    fun `single registry with no duplicates passes through unchanged`() {
        ...
        val registry = object : PreviewRegistry { override val entries = entries }
